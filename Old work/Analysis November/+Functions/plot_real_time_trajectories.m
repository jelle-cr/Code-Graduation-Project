function plot_real_time_trajectories(p, range, t_stop, t_step, delay)
    ...
    for t_ind = 1:1:(t_stop/t_step + 1)
        ...
        p_o = [3-1.2*t_ind*t_step; 2-1.1*t_ind*t_step];
        x_circle = r_o * cos(th) + p_o(1);
        y_circle = r_o * sin(th) + p_o(2);
        if handles_obs == 0
            handles_obs = patch(x_circle, y_circle, colors(1,:), ...
                                              'FaceColor', 'black', ...
                                              'FaceAlpha', 0.2, ...
                                              'EdgeColor', 'black', ...
                                              'HandleVisibility', 'off');
        else
            set(handles_obs, 'XData', x_circle, ...
                                'YData', y_circle);
        end

        % Update time in title
        title(['Trajectory ($t$ = ', sprintf('%.2f', t_ind*t_step-t_step), ' [s])'], ...
          'Interpreter', 'latex', 'FontSize', 18);
